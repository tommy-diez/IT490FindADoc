	...
	for ip in ip_list:
		...
		if p:
			...
			def send_select_to_office_2(): # get off_id now that office is inserted to use in inserting to link table
				...
				query = f"SELECT off_id from public.office where name='{office}'
				channel_publish.basic_publish(exchange='amq.direct',routing_key='beTOdbRK',body=query)
				print("Sent to db: " + query)
				consume_from_office_select_2() # consume response
			
			def consume_from_office_insert():
				channel_consume.basic_consume(queue="dbTObe", on_message_callback=send_select_to_office_2, auto_ack=True
			...
			def callback_from_office_select(ch, method, properties, body):
				...
				if body == "": # if it doesn't exist
					...
					name = officeNames[curr_index]
					query = f"INSERT INTO public.office(street, town, zip, state, phone, name) VALUES ('{street}', '{city}', '{state}', '{zip_code}', '{name}')"
			...
			def consume_from_insurance_insert():
				channel_consume.basic_consume(queue="dbTObe", on_message_callback=send_select_to_insurance, auto_ack=True)
